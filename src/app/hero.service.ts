...
export class HeroService {
  ...
  getMarvelHeroes(): Observable<Hero[]> {
    return this.http.get<Hero[]>('api/marvel').pipe(
      tap((_) => this.log('fetched marvel heroes')),
      catchError(this.handleError<Hero[]>('getMarvelHeroes', []))
    );
  }

  getMarvelHero(id: number): Observable<Hero> {
    const url = `api/marvel/${id}`;
    return this.http.get<Hero>(url).pipe(
      tap((_) => this.log(`fetched marvel hero id=${id}`)),
      catchError(this.handleError<Hero>(`getMarvelHeroe id=${id}`))
    );
  }
